...
sample <- "GFP_m3_1st_round"

input.datadir <- params$input.datadir
path.to.VDJ.output <- file.path(dirname(input.datadir), "VDJ_output")

path.to.input <- file.path(input.datadir, sample, "s8a_output")
path.to.output <- file.path(input.datadir, sample)
...
path.to.anno.contigs <- file.path(path.to.VDJ.output, sprintf("annotated_contigs_clonaltype_%s.csv", str_replace(sample, "_1st_round", "")))

path.to.count.clonaltype <- file.path(path.to.VDJ.output, sprintf("count_clonaltype_%s.csv", str_replace(sample, "_1st_round", "")))
...
### Cluster 8
```{r results='asis', echo=FALSE, message=FALSE, warning=FALSE}
subset(diff.cluster.markers, diff.cluster.markers$cluster == 8 & diff.cluster.markers$p_val_adj < 0.05) %>% 
  rowwise %>% 
  mutate_if(is.numeric, round, 6) %>% 
  subset(avg_log2FC > 0) %>%
  arrange(desc(avg_log2FC)) %>%
...
### Cluster 9
```{r results='asis', echo=FALSE, message=FALSE, warning=FALSE}
subset(diff.cluster.markers, diff.cluster.markers$cluster == 9 & diff.cluster.markers$p_val_adj < 0.05) %>% 
  rowwise %>% 
  mutate_if(is.numeric, round, 6) %>% 
  subset(avg_log2FC > 0) %>%
  arrange(desc(avg_log2FC)) %>%
...
save_plot_dir <- file.path(dirname(input.datadir), "top20_clonaltypes_overlayUMAP")
...
## Heatmap for cluster markers, GEX data
...
top10 <- data.frame()
for (cluster.id in unique(s.obj@meta.data$seurat_clusters)){
  specific.cluster.markers <- cluster.markers$diff.markers %>% subset(cluster == cluster.id) %>% arrange(desc(avg_log2FC)) %>% head(10)
  top10 <- rbind(top10, specific.cluster.markers)
}

top10 <- top10[order(top10$cluster),]

gex_cluster_markers_heatmap <- DoHeatmap(object = s.obj, features = top10$gene, label = TRUE, draw.lines = T
) +
  scale_fill_viridis(direction = -1)

dir.create(file.path(input.datadir, sample, "heatmap_cluster_markers"), showWarnings = FALSE)
ggsave(plot = gex_cluster_markers_heatmap, width = 25, height = 16, units = "in",
       filename = file.path(input.datadir, sample, "heatmap_cluster_markers", sprintf("heatmap_cluster_markers_%s.png", sample)), device = "png", dpi = 350)
```
