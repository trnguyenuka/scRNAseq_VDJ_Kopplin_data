---
title: "VDJ data analysis 230316_Kopplin_Pabst, m367 vs m369"
author:
...
s.obj <- subset(s.obj, name %in% c("m367", "m369"))
```
...
unique.clones[["m367"]] <- subset(all.clone.df, all.clone.df$m367 != 0 & all.clone.df$m369 == 0)
unique.clones[["m369"]] <- subset(all.clone.df, all.clone.df$m367 == 0 & all.clone.df$m369 != 0)
```
...
for (sample.id in names(unique.clones)){
  ...
  for (group in names(split.list.of.clones)){
    cat(sprintf("#### Part %s \n", group))
    p <- plot_a_list_of_clonal_types(s.obj = s.obj, list_of_clonal_types = split.list.of.clones[[group]], sample.id1 = "m367", sample.id2 = "m369")
    print(p)
    cat("\n \n")
  }
}
...
```{r results='asis', echo=FALSE, message=FALSE, warning=FALSE}
shared2.clones <- hash()
shared2.clones[["m367_m369"]] <- subset(all.clone.df, all.clone.df$m367 != 0 & all.clone.df$m369 != 0 )
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
##### SAVE OBJECTS 
saveRDS(object = s.obj, file.path(path.to.02.output, "230316_Kopplin.m367_m369.seurat.obj.addedShannonEntropy.rds"))
