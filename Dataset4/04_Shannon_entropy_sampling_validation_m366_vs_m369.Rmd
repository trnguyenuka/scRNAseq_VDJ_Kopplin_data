---
title: "Cell sampling and Shannon entropy for the 230316 dataset, m366 vs m369"
author:
...
path.to.03.output <- file.path(path.to.main.output, "03_output_m366_vs_m369")
path.to.04.output <- file.path(path.to.main.output, "04_output_m366_vs_m369")

s.obj <- readRDS(file.path(path.to.02.output, "230316_Kopplin.m366_m369.seurat.obj.addedShannonEntropy.rds"))
...
colnames(clone.summary) <- c("X", "CTaa", "m366", "m369", "total", "status", "Shannon.entropy")
...
```{r dataset_230316_m366_m369_all_read_clone_shannon, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
...
```{r dataset_230316_m366_m369_sampling_shannon, ampling_shannon_entropies, echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=10, dpi=300, dev='svg'}
all.clone.sizes <- sort(unique(clone.summary$total))
dir.create(file.path(path.to.04.output, "validation_m366_vs_m369"), showWarnings = FALSE, recursive = TRUE)
##### Helper function
...
for (num.sampling.cells in all.clone.sizes){
  ...
  print(p)
  cat("\n \n")
  writexl::write_xlsx(validationdf, file.path(path.to.04.output, "validation_m366_vs_m369", sprintf("validation_Shannon_entropy_sampling_%s.4th_m366_vs_m369_Dataset.xlsx", num.sampling.cells)))
}
```
...
## All clones
```{r dataset_230316_m366_m369_double_y_all_clones, echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=14, dpi=300, dev='svg'}
coeff <- 1000
...
```{r dataset_230316_m366_m369_double_y_clone_in_all_2_samples, echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=14, , dpi=300, dev='svg'}
coeff <- 1000

subset.shared.2.samples <- subset(clone.summary, clone.summary$status == "shared_in_2_samples")
if (nrow(subset.shared.2.samples) == 0){
  p <- ggplot() + ggtitle("No shared clones to calculate MHI")
} else {
  p <- subset.shared.2.samples %>% ggplot(aes(x = reorder(CTaa, -total))) + 
  geom_point(aes(y = Shannon.entropy)) +  ylim(0, 1) + 
  geom_bar(aes(y = total/coeff), stat = "identity") +  
  theme(axis.text.x = element_blank()) + xlab("Clone") +
  scale_y_continuous(
...
## Unique clones
```{r dataset_230316_m366_m369_double_y_clone_in_unique_1_sample, echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=14, , dpi=300, dev='svg'}
coeff <- 1000
...
### All cells
```{r dataset_230316_m366_m369_UMAP_all_2_samples, echo=FALSE, warning=FALSE, message=FALSE, include=TRUE, results='asis', fig.height=10, fig.width=14, dpi=300, dev='svg'}
...
```{r dataset_230316_m366_m369_shannon, echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=14, dpi=300, dev='svg'}
...
```{r dataset_230316_m366_m369_clonesize_lt10, echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=14, dpi=300, dev='svg'}
...
```{r dataset_230316_m366_m369_clonesize_gt10, echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=14, dpi=300, dev='svg'}
meta.data <- s.obj@meta.data %>% rownames_to_column("barcode") %>% subset(select = c(barcode, CTaa, total)) %>%
...
```{r dataset_230316_m366_m369_clonesize_all, echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=14, dpi=300, dev='svg'}
FeaturePlot(object = s.obj, 
...
```{r dataset_230316_m366_m369_clonesize_all_logScale, echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=14, dpi=300, dev='svg'}
