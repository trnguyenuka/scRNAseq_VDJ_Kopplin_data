---
title: "VDJ data analysis 230316_Kopplin_Pabst, m366 vs m367"
author:
...
s.obj <- subset(s.obj, name %in% c("m366", "m367"))
```
...
unique.clones[["m366"]] <- subset(all.clone.df, all.clone.df$m366 != 0 & all.clone.df$m367 == 0)
unique.clones[["m367"]] <- subset(all.clone.df, all.clone.df$m366 == 0 & all.clone.df$m367 != 0)
```
...
for (sample.id in names(unique.clones)){
  ...
  for (group in names(split.list.of.clones)){
    cat(sprintf("#### Part %s \n", group))
    p <- plot_a_list_of_clonal_types(s.obj = s.obj, list_of_clonal_types = split.list.of.clones[[group]], sample.id1 = "m366", sample.id2 = "m367")
    print(p)
    cat("\n \n")
  }
}
...
```{r results='asis', echo=FALSE, message=FALSE, warning=FALSE}
shared2.clones <- hash()
shared2.clones[["m366_m367"]] <- subset(all.clone.df, all.clone.df$m366 != 0 & all.clone.df$m367 != 0 )
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
##### SAVE OBJECTS 
saveRDS(object = s.obj, file.path(path.to.02.output, "230316_Kopplin.m366_m367.seurat.obj.addedShannonEntropy.rds"))
