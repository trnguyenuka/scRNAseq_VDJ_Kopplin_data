---
title: "MHI 230316_Kopplin_Pabst, m366 vs m369"
author:
...
colnames(clone.summary) <- c("CTaa", "m366", "m369", "total", "Status", "Shannon.entropy")
...
### m366 vs m369
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
res.clones.in.3.samples[["m366_vs_m369"]] <- subset(clones.in.3.samples, clones.in.3.samples$m366 >= 10 & clones.in.3.samples$m369 >= 10) %>% rowwise %>%
  mutate(MHI = func03_pipeline_01_02(umap, clone1 = CTaa, clone2 = CTaa, sample1 = "m366", sample2 = "m369")) 
res.clones.in.3.samples[["m366_vs_m369"]] %>% mutate_if(is.numeric, round, 5) %>% create_dt()
...
### Clones shared in m366 and m369
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
res.clones.in.2.samples[["m366_vs_m369"]] <- subset(clones.in.2.samples, clones.in.2.samples$m366 >= 10 & clones.in.2.samples$m369 >= 10) %>% rowwise %>%
  mutate(MHI = func03_pipeline_01_02(umap, clone1 = CTaa, clone2 = CTaa, sample1 = "m366", sample2 = "m369")) 
res.clones.in.2.samples[["m366_vs_m369"]] %>% mutate_if(is.numeric, round, 5) %>% create_dt()
```


```{r, echo=FALSE, warning=FALSE, message=FALSE}
all.res.clones.in.2.samples <- res.clones.in.2.samples$m366_vs_m369
...
### Unique clones m366 vs unique clones m369

```{r, echo=FALSE, warning=FALSE, message=FALSE}
compare_unique[["m366_vs_m369"]] <- expand.grid("clone1"= unique.clones$m366$CTaa, "clone2" = unique.clones$m369$CTaa)

compare_unique[["m366_vs_m369"]] <- compare_unique[["m366_vs_m369"]] %>% as.data.frame %>% rowwise %>%
  mutate(MHI = func03_pipeline_01_02(umap, clone1 = clone1, clone2 = clone2, sample1 = "m366", sample2 = "m369"))

compare_unique[["m366_vs_m369"]]  %>% mutate_if(is.numeric, round, 5) %>% create_dt()
```
...
final.clonedf <- clone.summary.raw[, c("clone", "status", "m366", "m369", "total")]
colnames(final.clonedf) <- c("Clone", "Status", "m366", "m369", "total")
final.clonedf <- final.clonedf %>% rowwise() %>%
  ...
  arrange(desc(total))
assign_MHI_m366_vs_m369 <- function(Clone, Status){
  if (Status == "shared_in_2_samples"){
    if (Clone %in% res.clones.in.2.samples$m366_vs_m369$CTaa){
      MHI_m366_vs_m369 <- subset(res.clones.in.2.samples$m366_vs_m369, res.clones.in.2.samples$m366_vs_m369$CTaa == Clone)$MHI
    } else {
      MHI_m366_vs_m369 <- NA
    }
  } else {
  ...
  return(MHI_m366_vs_m369)
}

final.clonedf <- final.clonedf %>% rowwise() %>%
  mutate(MHI_m366_vs_m369 = assign_MHI_m366_vs_m369(Clone, Status))

writexl::write_xlsx(final.clonedf, file.path(path.to.save.final.clonedf, "Dataset4_m366_vs_m369.xlsx"))
